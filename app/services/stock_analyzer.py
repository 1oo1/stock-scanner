...
class StockAnalyzer:
    ...
    def calculate_indicators(self, df):
        ...
        try:
            # 计算移动平均线
            df["MA5"] = self.calculate_ema(
                df["close"], self.params["ma_periods"]["short"]
            )
            df["MA20"] = self.calculate_ema(
                df["close"], self.params["ma_periods"]["medium"]
            )
            df["MA60"] = self.calculate_ema(
    ...
    def get_ai_analysis(self, df, stock_code, market_type="A", stream=False):
        ...
        try:
            ...
            logger.info(
                f"开始AI分析中国{type_name}股票 {stock_code}, 流式模式: {stream}"
            )
            ...
            if not self.API_URL:
                ...
                return (
                    error_msg
                    if not stream
                    else (yield json.dumps({"error": error_msg}))
                )
            ...
            if stream:
                ...
                try:
                    ...
                    logger.debug(
                        f"请求载荷: {self._truncate_json_for_logging(payload)}"
                    )
                ...
                try:
                    response = requests.post(
                        api_url, headers=headers, json=payload, timeout=self.API_TIMEOUT
                    )
                ...
                if line:
                    ...
                    if line.startswith("data: "):
                        ...
                        try:
                            ...
                            logger.error(
                                f"JSON解析错误: {str(e)}, 行内容: {data_content}"
                            )
                            # 忽略无法解析的JSON
                            pass
                    else:
    ...
    def analyze_stock(self, stock_code, market_type="A", stream=False):
        ...
        try:
            logger.info(
                f"开始分析股票: {stock_code}, 市场: {market_type}, 流式模式: {stream}"
            )
    ...
    def scan_market(self, stock_list, min_score=60, market_type="A", stream=False):
        """扫描市场，寻找符合条件的股票"""
        logger.info(
            f"开始扫描市场，股票数量: {len(stock_list)}, 最低分数: {min_score}, 市场: {market_type}, 流式模式: {stream}"
        )
        ...
        if not stream:
            ...
            for stock_code in stock_list:
                try:
                    ...
                    if report["score"] >= min_score:
                        logger.info(
                            f"股票 {stock_code} 评分 {report['score']} >= {min_score}，添加到推荐列表"
                        )
                        recommendations.append(report)
                    else:
                        logger.debug(
                            f"股票 {stock_code} 评分 {report['score']} < {min_score}，不添加到推荐列表"
                        )
                except Exception as e:
            ...
            for stock_code in stock_list:
                stock_count += 1
                logger.debug(
                    f"流式分析股票 {stock_code} ({stock_count}/{len(stock_list)})"
                )
                try:
                    # 分析单只股票并获取流式结果
                    chunk_count = 0
                    for chunk in self.analyze_stock(
                        stock_code, market_type, stream=True
                    ):
                        chunk_count += 1
                        yield chunk
                    logger.debug(
                        f"股票 {stock_code} 流式分析完成，共 {chunk_count} 个块"
                    )
                except Exception as e:
                    error_msg = f"分析股票 {stock_code} 时出错: {str(e)}"
                    logger.error(error_msg)
                    ...
                    error_json = json.dumps(
                        {"stock_code": stock_code, "error": error_msg}
                    )
                    logger.info(f"流式错误输出: {error_json}")
                    yield error_json
